...
##' If the data does not have replicates, `repl` must be `NULL`.
    ...
    if(is.null(year)){
      ...
      if(is.null(reg)){
        ...
        if(is.null(repl)){
          ...
          Model = rstan::sampling(object = stanmodels$entrymean1, data = df_stan,iter = iter,
    ...
    if(is.null(year)) # No year effect ------------------------
    {
      if(is.null(reg)) # No region information -----------------------------
      {
        if(is.null(repl)){
          ...
          Model = rstan::sampling(object = stanmodels$entrymean2, data = df_stan, iter = iter,
                                  cores = cores, chains = chains, pars = pars,
                                  warmup = warmup, thin = thin, seed = seed,
                                  init = init, verbose = verbose,
                                  algorithm = algorithm, control = control,
