...
1- `mod` is the model fitted using `bayes_met`

2- `effects` indicates which effects `mod` has. Here, you may provide a string vector with the codes for each possible effect. The codes are:
...
If the model do not have a given effect, just remove it from the `effects` vector. 

3- `nenv` indicates the number of environments

4- `check.stan.diag` asks whether you want some diagnostic plots provided by `rstan` or not. Note that these plots are built in ggplot2, so they are customizable.

The function provides the maximum a posteriori values
