...
The second step is to fit a Multi-environment Bayesian model using the `bayes_met` function, and consequently, the `rstan` language implemented within it. There are some importantat details that we would like to stress here:
...
knitr::include_graphics("img/desenho.svg")
```
...
outs = extr_outs(data = maize, trait = "GY", gen = "Hybrid", model = mod,
                 ...
                 nenv = 16, res.het = TRUE, check.stan.diag = FALSE)
