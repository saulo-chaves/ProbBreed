...
...Or the histogram of the genotypic main effects:
...
#### Highest posterior density of genotypic main effects
...
Let $\Omega$ be a subset of the high-performance selected genotypes according to the intensity of selection. A given genotype $j$ will belong to $\Omega$ if its genotypic marginal value ($\hat{g}_j$) is high (or low) enough compared to its peers. We can emulate the occurrence of $S$ trials ($s = 1, 2, \dots, S$) leveraging from the Monte Carlo discretized sampling from posterior distributions of the fitted Bayesian models. Then, the probability of the $j^{\text{th}}$ genotype belonging to $\Omega$ is its ratio of success ($\hat{g}_j \in \Omega$) events over the total number of sampled events [$S = (\hat{g}_j \in \Omega) + (\hat{g}_j \notin \Omega)]$, as follows::

$$
Pr(\hat{g}_j \in \Omega \vert y) = \frac{1}{S} \sum_{s=1}^S I(\hat{g}_j^{(s)} \in \Omega \vert y)
$$
where $I(\hat{g}_j^{(s)} \in \Omega \vert y)$ is an indicator variable that can assume two values: (1) if $\hat{g}_j \in \Omega$ in the $s^{\text{th}}$ sample, and (0) otherwise. Note that the uncertainty associated with the distribution of the values from the genotypic posterior values are captured by accounting $\hat{g}_j \in \Omega$ in $S$. 
...
$$
Note that this equation is applied if the selection direction is to increase the trait value. If the aim is to decrease it (for example, plant height or the damage of a disease), you can set `increase = F`, and the following equation is employed: 
...
Probabilities of superior performance highlight experimental genotypes with high agronomic stability. For ecological stability (invariance), the probability of superior stability is more adequate. If we consider genotypes that have low genotype-by-environment interaction variance [$var(ge_{jk})$] as stable, the probability of superior stability is given by the following equation: 

$$
Pr[var(\widehat{ge}_{jk}) \in \Omega \vert y] = \frac{1}{S} \sum_{s=1}^S I[var(\widehat{ge}_{jk}^{(s)}) \in \Omega \vert y]
$$
where $I[var(\widehat{ge}_{jk}^{(s)}) \in \Omega \vert y]$ indicates if $var(\widehat{ge}_{jk}^{(s)})$ exists in $\Omega$ (1) or not (0). Note that this probability can only be computed across environments since it depends on $var(\widehat{ge}_{jk})$. Pairwise probabilities of superior stability are also possible in this context: 
...
Assuming that the genotypic main effects are independent of the variance of the genotype-by-environment interaction effects, the joint probability of superior performance and superior stability follows the same ideia as the probability of occurrence of two independent events: 
...
It is exactly the same idea as previous stated, but instead of using the marginal genotypic effect of each genotype, we use the conditional genotypic effect ($g_{jk} = g_j + ge_{jk}$): 
